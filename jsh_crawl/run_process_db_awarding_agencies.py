...
def start():
    ...
    try:
        with mysql_connection.cursor() as cursor:
            sql_select_aa = "SELECT * FROM `awarding_agencies` WHERE opportunity_id is null"
            ...
            for db_awarding_agencies in db_awarding_agencies_list:
                ...
                if (opportunity_id > 0):
                    ...
                    sql_insert_opp = "INSERT INTO `opportunities` (`resource_cn_id`, `state`, `update_time`) VALUES (%s, %s, %s)"
                    cursor.execute(sql_insert_opp,
                                   (cn_id, "CLOSED", time_now()))
                    mysql_connection.commit()
                    ...
                    print("after opportunity_id: ", opportunity_id)
                    if (opportunity_id > 0):
                        sql_update_aa = "update `awarding_agencies` set `opportunity_id` = %s where `resource_cn_id` = %s"
                        cursor.execute(sql_update_aa, (opportunity_id, cn_id))
                        mysql_connection.commit()
